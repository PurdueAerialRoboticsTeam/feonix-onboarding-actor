...
impl Brightspace {
    ...
    async fn handle_message(&mut self, msg: BrightspaceMessage) {
		println!("[Actor] Brightspace is running handle_message() with new BrightspaceMessage: {:?}", msg);
		match msg {
            ...
            BrightspaceMessage::SendAllToAdmin { reply_to } => {
                if let Some(ad) = &self.admin {
					...
					ad.submit_student_grades(self.underling_grades.clone()).await;
				}
				else {
					println!("[ACTOR]: Brightspace does not have Admin initialized so nothing happened");
				}
